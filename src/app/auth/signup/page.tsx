...
export default function SignupPage() {
  ...
  const strengthColor = ['', 'bg-red-500', 'bg-orange-400', 'bg-yellow-400', 'bg-accent-500'][passwordStrength]
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex gap-1">
                  {[1,2,3,4].map(n => (
                    <div key={n} className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                      passwordStrength >= n ? strengthColor : 'bg-slate-700'
                    }`} />
                  ))}
                </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              {form.confirm && (
                <div className={`absolute right-4 top-1/2 -translate-y-1/2 ${form.password === form.confirm ? 'text-accent-500' : 'text-red-400'}`}>
                  <CheckCircle size={16} />
                </div>
              )}
            </div>
          ...
          <button type="submit" disabled={loading}
            className="w-full mt-2 bg-gradient-to-r from-brand-500 to-brand-700 hover:from-brand-400 hover:to-brand-600 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold rounded-xl px-6 py-3.5 transition-all duration-200 flex items-center justify-center gap-2 glow-brand">
